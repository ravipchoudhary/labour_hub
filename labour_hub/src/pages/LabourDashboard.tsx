...
export default function LabourDashboard() {
  ...
  const [hireActionLoadingId, setHireActionLoadingId] = useState<string | null>(null);
  ...
  const fetchAll = async () => {
    try {
      ...
      const profileRes = await axios.get("http://localhost:4000/api/labour/profile", {
      ...
      const jobsRes = await axios.get("http://localhost:4000/api/labour/jobs", {
        headers: { Authorization: `Bearer ${token}` },
      });

      const completed = jobsRes.data.completedJobs || [];
      setCompletedJobs(completed);

      const hireRes = await axios.get("http://localhost:4000/api/hire/pending", {
        headers: { Authorization: `Bearer ${token}` },
      });

      const pendingReqs: HireRequest[] = hireRes.data?.requests || hireRes.data || [];
      setHireRequests(pendingReqs);

      const rejectedCount = hireRes.data?.rejectedCount ?? 0;
      ...
      completed.forEach((job: Job) => {
        const month = job.month;
        if (!month) return;

        const idx = monthlyEarnings.findIndex((m) => m.month === month);
        if (idx >= 0) monthlyEarnings[idx].earning += job.amount;
      });
  ...
  const updateHireStatus = async (requestId: string, status: "accepted" | "rejected") => {
    try {
      ...
      await axios.patch(
        `http://localhost:4000/api/hire/${requestId}/status`,
        { status },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setJobStats((prev) => {
        ...
        if (pendingIndex >= 0) updated[pendingIndex].value = Math.max(0, updated[pendingIndex].value - 1);
        if (status === "rejected" && rejectedIndex >= 0) updated[rejectedIndex].value += 1;
  ...
  const totalEarnings = completedJobs.reduce((acc, job) => acc + job.amount, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Stat title="Total Jobs" value={`${completedJobs.length + hireRequests.length}`} />
        <Stat
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={earningsData}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="earning" stroke="#f97316" strokeWidth={3} />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="space-y-4">
            {completedJobs.length === 0 ? (
              ...
              completedJobs.map((job) => (
                <JobCard key={(job.id || job._id || Math.random()).toString()} job={job} />
              ))
            )}
          </div>
        </div>
